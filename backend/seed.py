...
VALORES_ESTRELAS = [Decimal("1.0"), Decimal("1.5"), Decimal("2.0"), Decimal("2.5"), Decimal("3.0"), Decimal("3.5"), Decimal("4.0"), Decimal("4.5"), Decimal("5.0")]
...
def _imprimir_progresso(rotulo: str, atual: int, total: int, inicio: float) -> None:
  ...
  sys.stdout.write(f"\r{rotulo}: {atual}/{total} ({percentual:5.1f}%) - {decorrido:,.1f}s")
  sys.stdout.flush()
  if atual >= total:
    sys.stdout.write("\n")
    sys.stdout.flush()
...
def popular_conteudos(session: Session, quantidade: int, tamanho_lote: int) -> list[int]:
  """Popula a tabela de conteúdos e retorna os IDs criados."""
  if quantidade <= 0:
    return []

  ultimo_id_externo = session.scalar(select(func.coalesce(func.max(Conteudo.id_externo), 0))) or 0
...
def popular_avaliacoes(session: Session, quantidade: int, tamanho_lote: int) -> list[int]:
  ...
  if not usuario_ids or not conteudo_ids:
    raise ValueError("É necessário popular usuários e conteúdos antes das avaliações.")
  ...
  for indice in range(quantidade):
    avaliacao = Avaliacao(
      ...
      comentario=(fake.sentence(nb_words=random.randint(8, 16)) if random.random() < 0.7 else None),
    )
...
def popular_favoritos(session: Session, quantidade: int, tamanho_lote: int) -> list[int]:
  ...
  conteudo_ids = _obter_ids(session, Conteudo)
  if not usuario_ids or not conteudo_ids:
    raise ValueError("É necessário popular usuários e conteúdos antes dos favoritos.")
...
def popular_assistidos(session: Session, quantidade: int, tamanho_lote: int) -> list[int]:
  ...
  conteudo_ids = _obter_ids(session, Conteudo)
  if not usuario_ids or not conteudo_ids:
    raise ValueError("É necessário popular usuários e conteúdos antes dos assistidos.")
...
def main() -> None:
  ...
  parser.add_argument("--usuarios", type=int, default=1000, help="Quantidade de usuários a criar")
  parser.add_argument("--conteudos", type=int, default=2000, help="Quantidade de conteúdos a criar")
  parser.add_argument("--avaliacoes", type=int, default=5000, help="Quantidade de avaliações a criar")
  parser.add_argument("--favoritos", type=int, default=3000, help="Quantidade de favoritos a criar")
  parser.add_argument("--assistidos", type=int, default=3000, help="Quantidade de assistidos a criar")
  parser.add_argument("--batch", type=int, default=1000, help="Tamanho do lote para inserção")
